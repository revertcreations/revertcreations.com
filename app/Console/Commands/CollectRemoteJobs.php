    ...
    protected function persistJobs(Collection $jobs): Collection
    {
        ...
        foreach ($jobs as $job) {
            ...
            if (! $opportunity->exists) {
                $slugSource = trim(($job['company'] ?? 'opportunity').' '.($job['title'] ?? 'role'));
                $opportunity->slug = Str::slug(Str::limit($slugSource, 80).'-'.Str::random(6));
            }

            $existingLinks = is_array($opportunity->links) ? $opportunity->links : [];
            $existingPayload = is_array($opportunity->ingest_payload) ? $opportunity->ingest_payload : [];

            $opportunity->fill([
                'company_name' => $job['company'] ?? null,
                'role_title' => $job['title'],
                'status' => $opportunity->status ?? Opportunity::STATUS_OPEN,
                'workflow_state' => $opportunity->workflow_state ?? 'sourced',
                'priority' => $opportunity->priority ?? 'medium',
                'source' => $job['source'],
                'source_channel' => $job['source_channel'] ?? null,
                'public_visibility' => $opportunity->public_visibility ?? false,
                'summary' => Str::limit($job['description'] ?? '', 600),
                'next_action_at' => $opportunity->next_action_at,
                'last_action_at' => $opportunity->last_action_at ?? Carbon::now(),
                'links' => array_filter(array_merge($existingLinks, [
                    'apply' => $job['apply_url'] ?? $job['url'],
                    'origin' => $job['url'],
                ])),
                'is_remote' => true,
                'ingest_status' => $opportunity->ingest_status ?? 'drafted',
                'ingest_payload' => array_filter(array_merge($existingPayload, [
                    'snapshot' => $job,
                ])),
            ]);
