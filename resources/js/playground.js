const Playground = {
    ...
    positionElement: (skill, bounds = null) => {
        while (!skill.isPositioned) {
            ...
            const textXBound =
                playgroundBounds.paddingLeft +
                (maxX > 0 ? Math.random() * maxX : 0)
            const textYBound =
                playgroundBounds.paddingTop +
    ...
    closeInfoCard: skill_name => {
        ...
        if (
            skill.elementChild &&
            skill.elementChild.parentNode === skill.element
        ) {
        ...
        if (
            skill.closeInfoCard &&
            skill.closeInfoCard.parentNode === skill.element
        ) {
    ...
    showHireForm: () => {
        ...
        organizationLabel.classList.add(
            'text-gruvbox-green',
            'w-full',
            'text-left',
            'mt-4'
