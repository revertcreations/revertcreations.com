Playground = {
    ...
    moreInfoTarget: document.getElementById('more_info_target'),
    skills: [],
    needsReset: false,
    fontScale: 25,
    placedSkills: [],
    ...
    init: (data) => {
        ...
        window.onresize = function(){
            clearTimeout(doit);
            doit = setTimeout(Playground.reset(), 500);
        };
    ...
    dragHereOverlaps: (skill) => {
        let rect2 = Playground.moreInfoTarget.getBoundingClientRect()
        let rect1 = skill.element.getBoundingClientRect()
    ...
    drag: (e) =>  {
        ...
        Playground.moreInfoTarget.innerText = 'Drag HERE!'

        Playground.moreInfoTarget.classList.add('border-dashed', 'border-4', 'border-light-blue-500')
        ...
        if (skill && skill.active) {
            ...
            if(Playground.dragHereOverlaps(skill)){
                Playground.currentSkill = skill
                e.stopPropagation()
                Playground.dragEnd(e)
                Playground.reset(function(){
                    Playground.displayInfoCard(Playground.currentSkill)
                })
                return false;
            }
    ...
    buildInfoCard: (skill) => {
        console.log('building info card', skill)
    ...
    displayInfoCard: (skill) => {
        ...
        if(skill.name != 'README.md' && skill.name != 'hire me') {
            Playground.playground.appendChild(skill.elementChildExperienceWrap)
