...
export class HintElement extends HTMLElement {
    ...
    glimmerHint = () => {
        ...
        this.#hintInterval = setInterval(this.glimmerHint, this.#hintEvery)
        this.#hintCount++
    }
        ...
        if (!this.#scoring) {
            ...
            fetch(checkUrl.toString(), {
                ...
                .then(response => {
                    if (!response.ok) {
                        throw new Error(
                            `Puzzle check failed (${response.status})`
                        )
                    }
                ...
                .then(json => {
                    ...
                    return fetch(solvedUrl.toString(), {
                        ...
                        .then(response => {
                            if (!response.ok) {
                                throw new Error(
                                    `Puzzle solve failed (${response.status})`
                                )
                            }
        ...
        result.leaderboard.forEach(entry => {
            ...
            const time =
                typeof entry.time === 'number'
                    ...
                    : entry.time
            right.textContent = `${hints} hint${hints === 1 ? '' : 's'}, ${
                time ?? '0.0'
            }s`
