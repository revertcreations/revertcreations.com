<x-admin-layout>
    <header class="bg-white shadow">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold text-gray-900">Add Auction Source</h1>
        </div>
    </header>
    ...
    <main>
        <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('admin.auction-sources.store') }}" class="space-y-8">
                @csrf
                            ...
                            <div>
                                ...
                                <select name="driver" id="driver" required
                                    ...
                                    @foreach($drivers as $key => $class)
                                        <option value="{{ $key }}" {{ old('driver') == $key ? 'selected' : '' }}>
                                            {{ ucfirst($key) }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="px-4 py-5 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="number" name="frequency_minutes" id="frequency_minutes" value="{{ old('frequency_minutes', 60) }}" min="1" required
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>

                            <div>
                                <label for="buyer_premium_percent" class="block text-sm font-medium text-gray-700">Default Buyer Premium (%)</label>
                                <input type="number" name="buyer_premium_percent" id="buyer_premium_percent" value="{{ old('buyer_premium_percent', 10) }}" min="0" max="100" step="0.01"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>

                            <div>
                                <label for="tax_rate_percent" class="block text-sm font-medium text-gray-700">Default Tax Rate (%)</label>
                                <input type="number" name="tax_rate_percent" id="tax_rate_percent" value="{{ old('tax_rate_percent', 0) }}" min="0" max="100" step="0.01"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <textarea name="filters" id="filters" rows="4"
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                          placeholder='{"categories": [], "keywords": []}'>{!! old('filters') !!}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="meta" id="meta" rows="3"
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                          placeholder='{"description": "Additional info"}'>{!! old('meta') !!}</textarea>
