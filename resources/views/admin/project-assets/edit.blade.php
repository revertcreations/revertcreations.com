<x-admin-layout>
    ...
    <header class="bg-white shadow">
        <div class="flex justify-between max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Project Asset</h1>
                <p class="mt-1 text-sm text-gray-500">{{ $asset->title ?? $asset->url }}</p>
            </div>
            <a href="{{ route('admin.project-assets.index') }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
                Back to assets
            </a>
        </div>
    </header>
    ...
    <main>
        <div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8 space-y-8">
            ...
            <form method="POST" action="{{ route('admin.project-assets.update', $asset) }}" class="space-y-8">
                @csrf
                @method('PUT')

                @include('admin.project-assets._form', ['asset' => $asset, 'projects' => $projects, 'types' => $types])
            ...
            <form method="POST" action="{{ route('admin.project-assets.destroy', $asset) }}" onsubmit="return confirm('Archive this asset?')" class="flex justify-end">
                @csrf
                @method('DELETE')
