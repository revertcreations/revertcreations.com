<x-admin-layout>
    ...
    <header class="bg-white shadow">
        <div class="flex justify-between max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            ...
            <a href="{{ route('project-assets.index') }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
                Back to assets
            </a>
        </div>
    </header>
    ...
    <main>
        <div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8 space-y-8">
            ...
            <form method="POST" action="{{ route('project-assets.update', $asset) }}" class="space-y-8">
                @csrf
                @method('PUT')
            ...
            <form method="POST" action="{{ route('project-assets.destroy', $asset) }}" onsubmit="return confirm('Archive this asset?')" class="flex justify-end">
                @csrf
                @method('DELETE')
