...
<main>
  ...
  &lt;section class="production-line-item"&gt;
    ...
    {% endfor %}
  &lt;/section&gt;
{% endfor %}@endverbatim</code></pre><p>If your existing template already loops through <code>line_items_in_shipment</code>, copy only the inner <code>for property</code> block into the item markup. Do not nest a second item loop accidentally.</p><p>The complete example is also available as a reusable, MIT-licensed file in the <a href="https://github.com/revertcreations/shopify-packing-slip-personalization">open-source GitHub repository</a>.</p></div></section>
